...
UpdateResult_Tpyedef sdcard_update_NP(void)
{
	u8 res=0;
...
UpdateResult_Tpyedef sdcard_update(void)
{
	...
	//校验固件
	fres=f_open(&crc_file, "0:/crc32.txt", FA_READ);
	fres=f_read(&crc_file, file_buffer, 10, &bytes_read);
	uint32_t crc_value_in_file = 0;
	sscanf((const char *)file_buffer, "0x%X", &crc_value_in_file);
//    printf("%d\r\n",crc_value_in_file);
//	crc_value_in_file=CRC32_file_check("0:/firmware.bin");
//	printf("%d\r\n",crc_value_in_file);
